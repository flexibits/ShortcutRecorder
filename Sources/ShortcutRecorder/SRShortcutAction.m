    ...
    switch (anEventType)
    {
        ...
        case NSEventTypeFlagsChanged:
        {
            ...
            if (keyCode == kVK_Command || keyCode == kVK_RightCommand)
                ...
                os_log_error(OS_LOG_DEFAULT, "#Error Unexpected key code %hu for the FlagsChanged event", keyCode);
            break;
        }
        default:
        ...
        @synchronized (self->_actions)
        {
            ...
            switch (GetEventKind(anEvent))
            {
                ...
                case kEventHotKeyReleased:
                    ...
                    break;
                default:
                    os_log_error(OS_LOG_DEFAULT, "#Error Unexpected key event of type %u", GetEventKind(anEvent));
                    return;
            }
            ...
            if (!actions.count)
            {
                os_log_error(OS_LOG_DEFAULT, "No actions for the shortcut");
                return;
            }
...
- (void)_registerHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    {
        os_log_error(OS_LOG_DEFAULT, "#Critical Failed to register Carbon hot key: %d, keyCode: %lu, modifierFlags: %lu", error, (unsigned long)aShortcut.keyCode, (unsigned long)aShortcut.modifierFlags);
        return;
    }

    os_log_debug(OS_LOG_DEFAULT, "Registered Carbon hot key %u, keyCode: %lu, modifierFlags: %lu", hotKeyID.id, (unsigned long)aShortcut.keyCode, (unsigned long)aShortcut.modifierFlags);
...
- (void)_unregisterHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    os_log_error(OS_LOG_DEFAULT, "Removing Carbon hot key %u", hotKeyID);
    OSStatus error = UnregisterEventHotKey(hotKey);

    if (error != noErr)
    {
        os_log_error(OS_LOG_DEFAULT, "#Critical Failed to unregister Carbon hot key %u, keyCode: %lu, modifierFlags: %lu: %d", hotKeyID, (unsigned long)aShortcut.keyCode, (unsigned long)aShortcut.modifierFlags, error);
    }
    else
    {
        os_log_debug(OS_LOG_DEFAULT, "Unregistered Carbon hot key %u, keyCode: %lu, modifierFlags: %lu", hotKeyID, (unsigned long)aShortcut.keyCode, (unsigned long)aShortcut.modifierFlags);
    }
...
+ (NSDictionary<NSString *, id> *)_parseSystemKeyBindings
{
    ...
    if (@available(macOS 10.13, *))
    {
        ...
        {
            os_log_error(OS_LOG_DEFAULT, "#Error unable to read system key bindings: %s", error.localizedDescription.UTF8String);
            systemKeyBindings = @{};
        }
    }
    else
...
+ (NSDictionary<NSString *, id> *)_parseUserKeyBindings
{
    ...
    if (@available(macOS 10.13, *))
    {
        ...
        if (!userKeyBindings)
        {
            os_log_debug(OS_LOG_DEFAULT, "#Error unable to read user key bindings: %s", error.localizedDescription.UTF8String);
            userKeyBindings = @{};
        }
    }
    else
